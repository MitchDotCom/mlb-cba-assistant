...
// Make text comparable (case/quotes/punctuation/spacing)
function norm(s) {
  return s
    .toLowerCase()
    .replace(/[“”]/g, '"')
...
// Read and cache the PDF text, page by page
...
// Score: exact substring wins; else word-overlap ratio
...
// Best page for one short quote (<= 40 words)
...
// Replace the "Verification" block with page links + one confidence line
async function attachVerification(answerText, pdfHref = '/mlb/MLB_CBA_2022.pdf') {
  ...
  // Pull up to 3 quotes (curly or straight quotes)
  const quotes = [];
  ...
  // Overall confidence = lowest of the per-quote confidences
