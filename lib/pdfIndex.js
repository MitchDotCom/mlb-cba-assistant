// lib/pdfIndex.js
// Deterministic server-side linker. NO topic hints.
// Chooses supporting page using: (1) quoted fragments, (2) user-question tokens, (3) bigrams.
// Prefers /public/mlb/cba_pages.json if present; falls back to PDF text via pdfjs-dist legacy build.
...
async function loadTextIndex(origin) {
  // Try JSON page index first (faster & stable)
  try {
...
function chooseBestPage(pages, question, answer) {
  ...
  if (best.score < 6) {
    // fallback to max overlap of top 10 tokens
...
function buildSnippet(pageText, question, answer, phrases) {
  ...
  // center near densest region of question tokens
...
function injectBlocks(answerText, citationLine, sourceBlock) {
  // Replace any existing "Citation:" blob with our canonical version.
  const rx = /(?:^|\n)Citation:[\s\S]*$/i;
...
async function attachVerification(answerText, questionText, pdfHref = "/mlb/MLB_CBA_2022.pdf", origin = "https://mlb.mitchleblanc.xyz") {
  ...
  // 2) Choose best page (NO topic hints)
