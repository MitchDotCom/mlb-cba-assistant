...
// Robust ESM import for pdfjs-dist on Vercel/Next; no worker needed on Node.
...
async function getPdfjs() {
  // Prefer the stable public build entry (works with pdfjs-dist v3/v4 on Vercel)
  // If that fails, fall back to package root.
  try {
    const mod = await import('pdfjs-dist/build/pdf'); // <-- main public build
...
async function loadPdf() {
  ...
  if (typeof getDocument !== 'function') {
    throw new Error('pdfjs-dist getDocument() not available (ESM import failed)');
  }

  const loadingTask = getDocument({ data }); // no worker needed on Node
...
// Scoring: exact substring wins; otherwise token overlap ratio
...
async function attachVerification(answerText, pdfHref = '/mlb/MLB_CBA_2022.pdf') {
  // Find the Verification section appended by the Assistant
  ...
  // Overall confidence = the lowest of individual confidences
